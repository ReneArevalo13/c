...
The easiest approach to this assignment is to start with your implementation of Map using a linked list and then adapt it from 
a single head value to an array of head values that each point to an independent linked list. We use the getBucket() function 
to choose the appropriate linked list head from the array of head values.
*/
...
void __HashMap_put(struct HashMap* self, char *key, int value) {
    ...
    new->key = save_key;
    new->__next = NULL;
    new->__prev = self->__tails[bucket]->__next= new;
    if (self->__heads[bucket] == NULL) self->__heads[bucket] = new;
    if (self->__tails[bucket] == NULL) self->__tails[bucket] = new;
