...
We will rename member variables and functions to follow the Python convention that "private" variables start with a double underscore (a.k.a. dunder)
in C. We will also switch the method calls to be stored within the object structure. There is a good deal of discussion of this application in the 
...
int __Map_get(struct Map* self, char *key, int def)
{
    ...
    struct MapEntry* find;
    if (__Map_find(self, key)) {
        find = __Map_find(self, key);
        def = find->value;
    }
    return def;
}
